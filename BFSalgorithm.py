...
{   int num1 = Getrandom();
    ...
    for (int cmt = 0; cmt < finalstreetnumber; cmt++) {
        ...
        int pointcor;
        for (int count1=0;count1<((finallineseg+1)*2)+10;count1++){
            pointcor=Getrandom();
...
void output(){
    ...
    for(unsigned int u=0;u<randsegnubcol.size();u++){
        ...
        while (q<randsegnubcol[u]){
            ...
            cout<<" "<<"("<<streetcontainer[u][q].end.x<<","<<streetcontainer[u][q].end.y<<")";
            q=q+1;
        }
        cout<<endl;
    }cout<<"g"<<endl;
    ...
    while ((c = getopt(argc, argv, "s:n:l:c:")) != -1) {
        switch (c) {
            case 's':
                ...
                if (streetquantity < 2) {streetquantity = 10; }
                break;

            case 'n':
                cvalue = optarg;
                linequantity = atoi(cvalue);
                if (linequantity < 1) { linequantity = 5; }
                break;
            case 'l':
                cvalue = optarg;
                waitnumber = atoi(cvalue);
                if (waitnumber < 5) { waitnumber = 5; }
                break;
            case 'c':
                cvalue = optarg;
                rangenumber = atoi(cvalue);
                if (rangenumber < 1) { rangenumber = 20; }
                break;
            case '?':
